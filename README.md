> [!NOTE]
> 本仓库仅用于 Xposed 模块分发与更新。源码、问题反馈和讨论请访问主仓库：[nijika-boyfriend/BiliBlocker](https://github.com/nijika-boyfriend/BiliBlocker)。
> 
> This repository is only used for Xposed Modules Repo updates. Please visit the main repository at [nijika-boyfriend/BiliBlocker](https://github.com/nijika-boyfriend/BiliBlocker) for source code, issues, and discussions.

# BiliBlocker

LSPosed/Xposed 模块：为 B 站 `tv.danmaku.bili` 评论区和播放页添加「拉黑」按钮，点击后直接拉黑作者。

LSPosed/Xposed module: Adds a "Block" button to the comment section and playback page of Bilibili `tv.danmaku.bili`, allowing you to block the author directly with one click.

## 说明

目前仅适配 **[B 站 8.93.0](https://github.com/nijika-boyfriend/BiliBlocker/releases/download/v1.0.0/tv.danmaku.bili_8.93.0_apkmirror.com.apkm)** 版本。  

Currently only compatible with **[Bilibili 8.93.0](https://github.com/nijika-boyfriend/BiliBlocker/releases/download/v1.0.0/tv.danmaku.bili_8.93.0_apkmirror.com.apkm)** version.  

## 下载

您可以前往 [Releases 页面](https://github.com/nijika-boyfriend/BiliBlocker/releases/tag/v1.0.0) 下载本模块的安装包。

You can go to the [Releases page](https://github.com/nijika-boyfriend/BiliBlocker/releases/tag/v1.0.0) to download the installation package for this module.  
